...
@section('content')
    @if (session('success'))
        <div class="mb-4 sm:mb-6 rounded-xl border border-emerald-200 bg-emerald-50 px-4 py-3 text-sm text-emerald-800">
            {{ session('success') }}
        </div>
    @endif

    @php
        $hasActiveFilters = request()->hasAny(['job_post_id', 'is_active']) && 
                           (request('is_active') !== 'all' || request('job_post_id'));
        $activeFilterCount = 0;
    ...
    <div class="bg-white rounded-xl sm:rounded-2xl shadow-sm border border-gray-100 mb-4 sm:mb-6 overflow-hidden">
        ...
        <div class="px-4 sm:px-6 py-4 border-b border-gray-100 bg-gray-50">
            <div class="flex items-center justify-between flex-wrap gap-3">
                ...
                @if($hasActiveFilters)
                    <a href="{{ route('admin.self-interview.index') }}" 
                       class="inline-flex items-center gap-2 px-3 sm:px-4 py-2 text-xs sm:text-sm font-semibold text-red-600 hover:text-red-700 hover:bg-red-50 rounded-lg transition-colors">
        ...
        <!-- Filter Form -->
        <form method="GET" action="{{ route('admin.self-interview.index') }}" class="p-4 sm:p-6">
            ...
            <div class="flex items-center gap-3 mt-6 pt-4 border-t border-gray-100 flex-wrap">
                <button type="submit" 
                        class="inline-flex items-center gap-2 px-4 sm:px-6 py-2 sm:py-2.5 bg-teal-600 text-white rounded-xl hover:bg-teal-700 transition-colors font-semibold text-xs sm:text-sm shadow-sm">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                @if($hasActiveFilters)
                    <a href="{{ route('admin.self-interview.index') }}" 
                       class="inline-flex items-center gap-2 px-4 sm:px-6 py-2 sm:py-2.5 border border-gray-200 text-gray-700 rounded-xl hover:bg-gray-50 transition-colors font-semibold text-xs sm:text-sm">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12" />
                        </svg>
                        Clear All
    ...
    <div class="mb-4 sm:mb-6 bg-gradient-to-r from-teal-600 to-teal-700 rounded-xl shadow-lg p-4 sm:p-6 text-white">
        <div class="flex items-center justify-between flex-wrap gap-4">
            <div>
                ...
                <p class="text-teal-100 text-xs sm:text-sm">Matching your filter criteria</p>
            </div>
            <a href="{{ route('admin.self-interview.index') }}" 
               class="px-3 sm:px-4 py-2 bg-white/20 hover:bg-white/30 rounded-lg text-xs sm:text-sm font-semibold transition whitespace-nowrap">
                View All Questions
            </a>
        </div>
    </div>
    ...
    @if($hasActiveFilters)
    <div class="bg-teal-50 border border-teal-200 rounded-xl sm:rounded-2xl p-3 sm:p-4 mb-4 sm:mb-6">
        <div class="flex items-center flex-wrap gap-2">
            <span class="text-xs sm:text-sm font-semibold text-teal-900">Active Filters:</span>
            @if(request('job_post_id'))
                @if(request('job_post_id') === 'global')
                    <span class="inline-flex items-center gap-1.5 px-2 sm:px-3 py-1 sm:py-1.5 bg-white border border-teal-200 rounded-lg text-xs sm:text-sm text-teal-800">
                        <span class="font-medium">Job Post:</span>
                        <span class="font-semibold">Global Questions Only</span>
                        <a href="{{ route('admin.self-interview.index', array_merge(request()->except('job_post_id'), ['page' => 1])) }}" 
                ...
                @else
                    ...
                    @if($selectedJobPost)
                        <span class="inline-flex items-center gap-1.5 px-2 sm:px-3 py-1 sm:py-1.5 bg-white border border-teal-200 rounded-lg text-xs sm:text-sm text-teal-800">
                            <span class="font-medium">Job Post:</span>
                            <span class="font-semibold truncate max-w-[150px] sm:max-w-none">{{ $selectedJobPost->title }}</span>
                            <a href="{{ route('admin.self-interview.index', array_merge(request()->except('job_post_id'), ['page' => 1])) }}" 
                               class="ml-1 text-teal-600 hover:text-red-600 transition-colors" title="Remove this filter">
                                <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12" />
            ...
            @if(request('is_active') && request('is_active') !== 'all')
                <span class="inline-flex items-center gap-1.5 px-2 sm:px-3 py-1 sm:py-1.5 bg-white border border-teal-200 rounded-lg text-xs sm:text-sm text-teal-800">
                    ...
                    <a href="{{ route('admin.self-interview.index', array_merge(request()->except('is_active'), ['page' => 1])) }}" 
                       class="ml-1 text-teal-600 hover:text-red-600 transition-colors" title="Remove this filter">
                        <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12" />
                        </svg>
                        ...
                        <tr class="hover:bg-slate-50">
                            ...
                            <td class="px-3 sm:px-6 py-4 whitespace-nowrap text-right text-xs sm:text-sm font-medium">
                                <div class="flex items-center justify-end gap-2 flex-wrap">
                                    <a href="{{ route('admin.self-interview.edit', $question) }}" class="text-teal-600 hover:text-teal-800 whitespace-nowrap">Edit</a>
                                    <form action="{{ route('admin.self-interview.toggle-status', $question) }}" method="POST" class="inline">
                                        @csrf
                                        <button type="submit" class="text-slate-600 hover:text-slate-800 whitespace-nowrap">
                                            {{ $question->is_active ? 'Deactivate' : 'Activate' }}
                                        </button>
                                    </form>
                                    <form action="{{ route('admin.self-interview.destroy', $question) }}" method="POST" class="inline delete-question-form" data-question-id="{{ $question->id }}">
                                        @csrf
                                        @method('DELETE')
                    ...
                    @empty
                        <tr>
                            <td colspan="6" class="px-6 py-12 text-center text-slate-500 text-sm">
                                No questions found. <a href="{{ route('admin.self-interview.create') }}" class="text-teal-600 hover:text-teal-700">Create your first question</a>
                            </td>
                        </tr>
                    @endforelse
                </tbody>
    ...
    <!-- Bulk Actions Forms -->
    <form id="bulk-activate-form" action="{{ route('admin.self-interview.bulk-activate') }}" method="POST" style="display: none;">
        @csrf
        <input type="hidden" name="question_ids" id="bulk-activate-ids">
    </form>

    <form id="bulk-deactivate-form" action="{{ route('admin.self-interview.bulk-deactivate') }}" method="POST" style="display: none;">
        @csrf
        <input type="hidden" name="question_ids" id="bulk-deactivate-ids">
    </form>

    <form id="bulk-delete-form" action="{{ route('admin.self-interview.bulk-delete') }}" method="POST" style="display: none;">
        @csrf
        @method('DELETE')
