    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5 mb-6">
        <div class="group relative bg-gradient-to-br from-white to-slate-50 rounded-2xl shadow-md hover:shadow-xl transition-all duration-300 p-6 border border-slate-200/60 overflow-hidden">
            ...
            <div class="relative">
                <div class="flex items-center justify-between mb-4">
                    <div class="w-12 h-12 rounded-xl bg-gradient-to-br from-teal-500 to-emerald-600 flex items-center justify-center shadow-lg">
                ...
                <p class="text-4xl font-bold text-slate-900">{{ $totalBranchesCount }}</p>
            </div>
        </div>
        <div class="group relative bg-gradient-to-br from-white to-emerald-50/30 rounded-2xl shadow-md hover:shadow-xl transition-all duration-300 p-6 border border-emerald-200/60 overflow-hidden">
            ...
            <div class="relative">
                ...
                <p class="text-4xl font-bold text-emerald-700">{{ $activeBranchesCount }}</p>
            </div>
        </div>
        <div class="group relative bg-gradient-to-br from-white to-amber-50/30 rounded-2xl shadow-md hover:shadow-xl transition-all duration-300 p-6 border border-amber-200/60 overflow-hidden">
            ...
            <div class="relative">
                ...
                <p class="text-sm font-medium text-slate-600 mb-1">Hidden Branches</p>
                <p class="text-4xl font-bold text-amber-600">{{ $hiddenBranchesCount }}</p>
            </div>
        </div>
        <div class="group relative bg-gradient-to-br from-white to-purple-50/30 rounded-2xl shadow-md hover:shadow-xl transition-all duration-300 p-6 border border-purple-200/60 overflow-hidden">
            ...
            <div class="relative">
                ...
                <p class="text-4xl font-bold text-purple-600">{{ $filteredBranchesCount }}</p>
            </div>
        </div>
    </div>

    <!-- Filters -->
    <form method="GET" action="{{ route('admin.branches.index') }}" class="mb-6">
        <div class="bg-white rounded-2xl shadow-md border border-slate-200/60 p-5">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="text" name="search" value="{{ request('search') }}" placeholder="Search by name, address, city, or phone..." 
                           class="w-full px-4 py-2.5 border border-slate-200 rounded-xl focus:ring-2 focus:ring-teal-500 focus:border-transparent text-sm transition-all">
                </div>
                <div>
                    <label class="block text-sm font-semibold text-slate-700 mb-2">Status</label>
                    <select name="is_active" class="w-full px-4 py-2.5 border border-slate-200 rounded-xl focus:ring-2 focus:ring-teal-500 focus:border-transparent text-sm transition-all">
            ...
            <div class="flex items-center gap-3 mt-5">
                <button type="submit" class="px-5 py-2.5 bg-teal-600 text-white rounded-xl hover:bg-teal-700 transition-colors font-semibold text-sm shadow-sm">
                    Apply Filters
                </button>
                @if(request()->hasAny(['search', 'is_active']))
                    <a href="{{ route('admin.branches.index') }}" class="px-5 py-2.5 border border-slate-200 text-slate-600 rounded-xl hover:bg-slate-50 transition-colors font-semibold text-sm">
                        Clear Filters
                    </a>
                @endif
            </div>
    ...
    <div class="bg-white border border-slate-200/60 rounded-2xl shadow-md overflow-hidden">
        ...
        <table class="w-full text-left text-sm min-w-[640px]">
            <thead class="bg-gradient-to-r from-slate-50 to-slate-100 text-slate-600 uppercase tracking-wide text-xs font-semibold">
                <tr>
                    <th class="px-4 sm:px-6 py-4">Branch</th>
                    <th class="px-4 sm:px-6 py-4">Location</th>
                    <th class="px-4 sm:px-6 py-4 hidden sm:table-cell">Phones</th>
                    ...
                    <tr class="hover:bg-slate-50/50 transition-colors duration-150">
                        <td class="px-4 sm:px-6 py-5">
                            <div class="flex items-center gap-3">
                                <div class="w-10 h-10 rounded-xl bg-gradient-to-br from-teal-100 to-emerald-100 border-2 border-teal-200 flex items-center justify-center flex-shrink-0 shadow-sm">
                                    <svg class="w-5 h-5 text-teal-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 11a3 3 0 11-6 0 3 3 0 016 0z"/>
                                    </svg>
                                </div>
                                <div>
                                    <p class="font-bold text-slate-900 text-sm sm:text-base">{{ $branch->name }}</p>
                                    <p class="text-xs text-slate-400 mt-0.5">Updated {{ $branch->updated_at->diffForHumans() }}</p>
                                </div>
                            </div>
                        </td>
                        <td class="px-4 sm:px-6 py-5 text-slate-700 text-sm">
                            <div class="space-y-0.5">
                                <p class="font-medium">{{ $branch->address_line1 }}</p>
                                @if($branch->address_line2)<p class="text-slate-600">{{ $branch->address_line2 }}</p>@endif
                                @if($branch->city)
                                    <div class="flex items-center gap-1.5 mt-1">
                                        <svg class="w-3.5 h-3.5 text-slate-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 11a3 3 0 11-6 0 3 3 0 016 0z"/>
                                        </svg>
                                        <p class="text-slate-600">{{ $branch->city }}</p>
                                    </div>
                                @endif
                            </div>
                        </td>
                        <td class="px-4 sm:px-6 py-5 text-slate-600 text-sm hidden sm:table-cell">
                            <div class="space-y-1.5">
                                @if($branch->phone_primary)
                                    <div class="flex items-center gap-1.5">
                                        ...
                                        <p class="font-medium">{{ $branch->phone_primary }}</p>
                                    </div>
                                @endif
                                @if($branch->phone_secondary)
                                    <div class="flex items-center gap-1.5">
                                        ...
                                        <p>{{ $branch->phone_secondary }}</p>
                                    </div>
                                @endif
                            </div>
                        </td>
                        <td class="px-4 sm:px-6 py-5 text-slate-600 text-sm hidden md:table-cell font-medium">{{ $branch->display_order }}</td>
                        <td class="px-4 sm:px-6 py-5">
                            <span class="inline-flex items-center px-3 py-1.5 rounded-lg text-xs font-semibold {{ $branch->is_active ? 'bg-emerald-100 text-emerald-700 border border-emerald-200' : 'bg-slate-100 text-slate-700 border border-slate-200' }}">
                                {{ $branch->is_active ? 'Visible' : 'Hidden' }}
                            </span>
                        </td>
                        <td class="px-4 sm:px-6 py-5 text-right">
                            <div class="flex items-center justify-end gap-2 sm:gap-3">
                                <a href="{{ route('admin.branches.edit', $branch) }}" class="inline-flex items-center gap-1 px-3 py-1.5 rounded-lg text-xs sm:text-sm font-semibold text-emerald-600 hover:bg-emerald-50 transition-colors">
                    ...
                    <tr>
                        <td colspan="6" class="px-6 py-12 text-center">
                            <div class="flex flex-col items-center justify-center">
                                ...
                                <p class="text-slate-500 font-medium text-base mb-2">No branches yet</p>
                                <p class="text-slate-400 text-sm mb-4">Get started by adding your first branch</p>
                                <a href="{{ route('admin.branches.create') }}" class="inline-flex items-center gap-2 px-4 py-2 bg-teal-600 text-white rounded-xl hover:bg-teal-700 transition-colors font-semibold text-sm">
