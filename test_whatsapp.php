...
try {
    ...
    $message = new ContactMessageReply([
        ...
        'message' => 'Test WhatsApp from Fortress Lenders - ' . date('Y-m-d H:i:s'),
        'channel' => 'whatsapp',
        'status' => 'pending'
    ]);
    
    $message->save();
    echo "Created test WhatsApp message with ID: " . $message->id . "\n";
    
    // Send WhatsApp
    $service = new MessagingService();
    $result = $service->send($message);
    
    echo "WhatsApp " . ($result ? "sent successfully" : "failed") . "\n";
